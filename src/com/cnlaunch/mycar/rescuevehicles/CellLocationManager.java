...
{
    public static int CHECK_INTERVAL = 15000; // 检查位置改变的时间间隔

    public static boolean ENABLE_WIFI = true; // Wifi是否可用

    private static boolean IS_DEBUG = false; // 是否调试

    private static final int STATE_COLLECTING = 2; // 连接状态

    private static final int STATE_IDLE = 0; // 空闲状态

    private static final int STATE_READY = 1; // 准备状态

    private static final int STATE_SENDING = 3; // 发送信息状态

    private static final int MESSAGE_INITIALIZE = 1; // 初始化信息

    private static final int MESSAGE_COLLECTING_CELL = 2; // 连接手机

    private static final int MESSAGE_COLLECTING_WIFI = 5; // 连接wifi网络
...
//        }
    ...
    private class CellLocationManagerBroadcastReceiver extends BroadcastReceiver
    {
        ...
        public void onReceive(Context arg0, Intent intent)
        {
            ...
            if (WifiManager.SCAN_RESULTS_AVAILABLE_ACTION.equals(s))
