...
public class ProtocolUtils {
	public final static byte DPU_ADDR = (byte)0xf0;
	public final static byte PHONE_ADDR = (byte)0xf8;
	// 0x55 0xAA 0xF0 0xF8
	 ...
	 * 过滤出返回的完整数据包
	 * */
	final static byte[] NONE={(byte)0xff,(byte)0xff,(byte)0xff,(byte)0xff}; 
...
//		System.out.println("命令字及名字参数:"+bytesToHexStringNoBar(command));
	...
	/**@author luxingsong
	 * 过滤出数据包中的命令字及命令参数字节 cmd  subcmd cmdparam
	 * */
...
//			System.out.println("OrderUtil 4字节校验数据包长度:["+checksum.length+"字节]:"+checkSumStr);
    ...
    */
	...
	/**@author luxingsong
	 * @see 字节数组操作 System.arraycopy()
	 * 将data数组的数据附加到src数组之后
	 * */
	public static byte[] appendByteArray(byte[] src,byte[] data)
	{
		if(src==null || data==null)
			throw new NullPointerException("appendByteArray() src 或者 data 参数包含空指针");
		if(src.length>0 && data.length>0)
		{
			 ...
			 return ret;
		}else throw new IllegalArgumentException("appendByteArray() src 或者 data 字节数组长度为0");
	}
}
